...
class Transformation:
    def __init__(
        ...
        if self.is_shifted:
            ...
            assert (
                type(shift_step) == np.ndarray
            ), "shift_step must be numpy array or None!"

            self.shifter = lambda arr: arr - shift_step
            self.unshifter = lambda arr: arr + shift_step
        else:
            self.shifter = lambda arr: arr
            self.unshifter = lambda arr: arr

        if self.is_rotated:
            if type(rotation_matrix) == np.ndarray:

                assert np.allclose(
                    rotation_matrix.T, np.linalg.inv(rotation_matrix)
                ), "Rotation matrix must be ortogonal!"

                # create rotator
                self.rotator = lambda arr: arr.dot(rotation_matrix)
            else:
                assert (
                    type(rotation_matrix) == int
                ), "rotation_matrix is not int dim and not a matrix!"
                # init rotator
                rotation_matrix, _ = np.linalg.qr(
                    np.random.random((rotation_matrix, rotation_matrix)),
                    mode="complete",
                )
                self.rotator = lambda arr: arr.dot(rotation_matrix)
                self.unrotator = lambda arr: arr.dot(rotation_matrix.T)
        else:
            self.rotator = lambda arr: arr
            self.unrotator = lambda arr: arr
            self.is_rotated = False

        self.noiser = (lambda val: val) if noise_generator is None else noise_generator

        self.f = lambda arr: self.noiser(
            transformed_function(self.rotator(self.shifter(arr)))
        )
        ...
        if not self.is_noised and hasattr(transformed_function, "x_best"):
            if not (transformed_function.x_best is None):
                self.x_best = self.unrotator(
                    self.unshifter(transformed_function.x_best)
                )
