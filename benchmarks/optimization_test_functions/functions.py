...
def check_dim(dim, min=1):
    assert (
        type(dim) == int and dim >= min
    ), f"Dimension should be int and not less than {min} for this function (got {dim})"
...
class AckleyTest:
    ...
    def __call__(self, vec):
        ...
        s = sum(
            (
                3 * (math.cos(2 * vec[i]) + math.sin(2 * vec[i + 1]))
                + self.exp * math.sqrt(vec[i] ** 2 + vec[i + 1] ** 2)
...
class Rosenbrock:
    ...
    def __call__(self, vec):
        ...
        s = sum(
            (
                100 * (vec[i + 1] - vec[i] ** 2) ** 2 + (vec[i] - 1) ** 2
                for i in range(vec.size - 1)
            )
        )
...
class Fletcher:
    ...
    def __init__(self, dim, seed=None):
        ...
        self.A = np.sum(
            self.a * np.sin(self.x_best) + self.b * np.cos(self.x_best), axis=0
        )
...
class Penalty2:
    ...
    def __call__(self, vec):
        ...
        s1 = 10 * math.sin(self.pi3 * vec[0]) ** 2 + (vec[-1] - 1) ** 2 * (
            1 + math.sin(self.pi2 * vec[-1] ** 2)
        )

        s2 = sum(
            (
                (vec[i] - 1) ** 2 * (1 + math.sin(self.pi3 * vec[i + 1] ** 2))
                for i in range(vec.size - 1)
            )
        )
...
class Michalewicz:
    ...
    def __call__(self, vec):
        ...
        return -sum(
            (
                math.sin(x) * math.sin((i + 1) * x**2 / math.pi) ** self.m
...
class Scheffer:
    ...
    def __call__(self, vec):
        ...
        return 0.5 + sum(
            (
                (math.sin(vec[i] ** 2 - vec[i + 1] ** 2) ** 2 - 0.5)
                / (1 + 0.001 * (vec[i] ** 2 + vec[i + 1] ** 2)) ** 2
                for i in range(vec.size - 1)
            )
        )
...
class Eggholder:
    ...
    def __call__(self, vec):
        ...
        return -sum(
            (
                (vec[i + 1] + 47)
                * math.sin(math.sqrt(abs(vec[i + 1] + vec[i] / 2 + 47)))
                + vec[i] * math.sin(math.sqrt(abs(vec[i] - vec[i + 1] - 47)))
                for i in range(vec.size - 1)
            )
        )
...
class Weierstrass:
    ...
    def __call__(self, vec):
        ...
        return self.bias + sum(
            (
                sum((ak * math.cos(x * pibk) for ak, pibk in zip(self.ak, self.pibk)))
                for x in vec * 2 + 1
            )
        )
